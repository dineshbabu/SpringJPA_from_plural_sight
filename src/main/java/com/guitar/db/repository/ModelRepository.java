...
public class ModelRepository {
	...
	public List<Model> getModelsInPriceRange(BigDecimal lowest, BigDecimal highest) {
		return modelJpaRepository.getByPriceGreaterThanEqualAndPriceLessThanEqual(lowest, highest);
	}
	...
	public List<Model> getModelsByPriceRangeAndWoodType(BigDecimal lowest, BigDecimal highest, String wood) {
		return modelJpaRepository.qyeryByPriceRanegAndWoodType(lowest, highest, "%"+wood+"%");
	}
	
	/**
	 * Custom finder with paging
	 */
	public Page<Model> getModelsByPriceRangeAndWoodTypeWithPaging(BigDecimal lowest, BigDecimal highest, String wood) {
		Sort sort = new Sort(Sort.Direction.ASC, "name");
		Pageable page = new PageRequest(0, 2, sort);
		return modelJpaRepository.qyeryByPriceRanegAndWoodTypeWithPaging(lowest, highest, "%"+wood+"%", page);
	}
